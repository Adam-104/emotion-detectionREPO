...
def detect_face_crop(img_bgr):
    ...
    return img_bgr[
        max(0, y - pad):min(img_bgr.shape[0], y + h + pad),
...
def _correct_age(raw_age, face_crop_bgr):
    ...
    print(
        f"Age correction: raw={raw_age:.1f} -> base={base} + "
        f"visual={visual_boost * 0.4:.1f} = {int(round(corrected))}"
    )
...
def get_age_gender(image_path):
    for detector in ["retinaface", "opencv"]:
        try:
            ...
            print(f"DeepFace ({detector}): raw={raw_age:.1f} -> {age_str}, {gender}")
